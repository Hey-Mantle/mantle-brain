...
- **⚠️ Decide your cap-blocked-fee policy deliberately.** When the usage cap is
  full at charge time, the usage-record post can fail — on Shopify,
  `appUsageRecordCreate` returns "Total price exceeds balance remaining". Decide
  explicitly what happens next: inspect the charge result and do **NOT** advance
  the period (or retry / raise the cap) when the post failed, so a blocked period
  isn't skipped and that cycle's fee isn't lost. Note `rollOverPendingUsage` does
  **not** govern this path — it only affects the separate metered-overage rollup.
